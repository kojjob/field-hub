defmodule FieldHubWeb.UserAuthTest do
  ...
  @remember_me_cookie "_field_hub_web_user_remember_me"
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      FieldHubWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :require_sudo_mode" do
    test "allows users that have authenticated in the last 30 minutes", %{conn: conn, user: user} do
      user_token = Accounts.generate_user_session_token(user)
      session = conn |> put_session(:user_token, user_token) |> get_session()
    ...
    test "redirects when authentication is too old", %{conn: conn, user: user} do
      thirty_one_minutes_ago = DateTime.utc_now(:second) |> DateTime.add(-31, :minute)
      user = %{user | authenticated_at: thirty_one_minutes_ago}
      user_token = Accounts.generate_user_session_token(user)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        FieldHubWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
