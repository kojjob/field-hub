defmodule FieldHubWeb.SettingsLive.Workflows do
  ...
  def render(assigns) do
    ...
    <div class="max-w-5xl mx-auto">
      <div class="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 class="text-3xl font-bold text-zinc-900 dark:text-white tracking-tight">Workflow Settings</h1>
        ...
        <div class="flex gap-2">
           ...
           <.button type="button" phx-click="save_all" variant="primary" class="gap-2 shadow-lg shadow-fsm-primary/20">
        ...
        <div class="lg:col-span-8 space-y-6">
          <div class="bg-white dark:bg-zinc-800/50 rounded-2xl shadow-sm border border-zinc-200 dark:border-zinc-700/50 overflow-hidden">
            <div class="p-4 border-b border-zinc-100 dark:border-zinc-700/50 bg-zinc-50/50 dark:bg-zinc-900/50 flex items-center justify-between">
              ...
              <span class="text-[10px] font-medium text-zinc-400 tracking-wider uppercase">Order by sequence</span>
            </div>
              ...
              <%= for {status, index} <- Enum.with_index(@statuses) do %>
                <div class="group p-4 flex items-center gap-4 hover:bg-zinc-50 dark:hover:bg-zinc-900/30 transition-colors">
                  <div class="flex flex-col items-center gap-1 text-zinc-300 dark:text-zinc-600">
                    <button phx-click="move_up" phx-value-index={index} class="hover:text-fsm-primary disabled:opacity-30" disabled={index == 0}>
                      <.icon name="hero-chevron-up" class="size-4" />
                    </button>
                    <button phx-click="move_down" phx-value-index={index} class="hover:text-fsm-primary disabled:opacity-30" disabled={index == length(@statuses) - 1}>
                      <.icon name="hero-chevron-down" class="size-4" />
                    </button>
                  </div>

                  <div class="size-3 rounded-full shrink-0 shadow-sm" style={"background-color: #{get_field(status, :color, "#6B7280")}"}></div>

                  <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2">
                      <span class="font-semibold text-zinc-900 dark:text-white truncate"><%= get_field(status, :label, "Status") %></span>
                  ...
                  <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button type="button" phx-click="edit_status" phx-value-index={index} class="p-2 text-zinc-500 hover:text-fsm-primary">
                      <.icon name="hero-pencil-square" class="size-4" />
                    </button>
                    <button type="button" phx-click="delete_status" phx-value-index={index} class="p-2 text-zinc-500 hover:text-red-500" data-confirm="Are you sure?">
            ...
            <%= if Enum.empty?(@statuses) do %>
              <div class="p-12 text-center">
                 ...
                 <p class="text-sm text-zinc-500 mt-1">Add your first status or apply a preset to get started.</p>
              </div>
            <% end %>
          </div>

          <!-- Visual Workflow Preview -->
          <div class="bg-white dark:bg-zinc-800/50 rounded-2xl shadow-sm border border-zinc-200 dark:border-zinc-700/50 p-6 overflow-hidden">
             <h2 class="text-base font-semibold text-zinc-900 dark:text-white mb-6">Workflow Sequence</h2>
                 ...
                 <div class="flex items-center">
                    <div class="relative group">
                       <div class="px-4 py-3 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 shadow-sm flex items-center gap-3">
                          <div class="size-2 rounded-full" style={"background-color: #{get_field(status, :color, "#6B7280")}"}></div>
                          <span class="text-xs font-bold text-zinc-700 dark:text-zinc-300 uppercase tracking-wider"><%= get_field(status, :label, "Status") %></span>
                       </div>
                    </div>
                    <%= if index < length(@statuses) - 1 do %>
                      <div class="w-8 h-px bg-zinc-200 dark:bg-zinc-700 relative">
                        <div class="absolute right-0 -top-[3px] border-t-4 border-b-4 border-l-4 border-t-transparent border-b-transparent border-l-zinc-200 dark:border-l-zinc-700"></div>
                      </div>
                    <% end %>
                 </div>
               <% end %>
        ...
        <div class="lg:col-span-4 space-y-6">
          <div class="bg-white dark:bg-zinc-800 rounded-2xl shadow-sm border border-zinc-200 dark:border-zinc-700/50 p-6">
            <h2 class="text-lg font-semibold text-zinc-900 dark:text-white mb-4 flex items-center gap-2">
              <.icon name="hero-sparkles" class="size-5 text-fsm-primary" /> Industry Templates
            </h2>
            ...
            <div class="space-y-3">
              <%= for preset <- @presets do %>
                <button
                  ...
                  class="w-full p-4 rounded-xl border border-zinc-100 dark:border-zinc-700 hover:border-fsm-primary hover:bg-fsm-primary/5 dark:hover:bg-fsm-primary/10 transition-all text-left group"
                >
                  <div class="flex items-center justify-between mb-1">
                    <span class="text-sm font-bold text-zinc-900 dark:text-white group-hover:text-fsm-primary"><%= preset.name %></span>
                    <.icon name="hero-arrow-path" class="size-3.5 text-zinc-400 opacity-0 group-hover:opacity-100 transition-opacity" />
                  </div>
          ...
          <div class="bg-blue-600/5 border border-blue-500/10 rounded-2xl p-6">
             <div class="flex gap-3">
                <.icon name="hero-light-bulb" class="size-6 text-blue-500 shrink-0" />
                <div>
                   <h3 class="text-sm font-bold text-blue-900 dark:text-blue-400">Pro Tip</h3>
                   <p class="text-xs text-blue-800/80 dark:text-blue-400/80 mt-1 leading-relaxed">
                     The order of statuses defines the default progression. Status keys like <span class="font-mono">completed</span> or <span class="font-mono">cancelled</span> are used for reporting.
                   </p>
                </div>
             </div>
          </div>
    ...
    <%= if @editing_status do %>
      <div class="fixed inset-0 bg-zinc-950/20 dark:bg-zinc-950/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" phx-click="close_editor">
        <div class="bg-white dark:bg-zinc-900 rounded-3xl shadow-2xl border border-zinc-200 dark:border-zinc-800 w-full max-w-lg overflow-hidden" phx-click-away="close_editor">
          ...
          <form phx-submit="save_status" class="px-8 pb-8 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label class="block text-sm font-semibold text-zinc-700 dark:text-zinc-300 mb-2">Display Label</label>
                <input
                  type="text"
                  name="label"
                  value={@editing_status.label}
                  class="w-full px-4 py-3 border border-zinc-300 dark:border-zinc-700 rounded-xl bg-zinc-50 dark:bg-zinc-800/50 focus:ring-2 focus:ring-fsm-primary outline-none transition-all"
              ...
              <div>
                <label class="block text-sm font-semibold text-zinc-700 dark:text-zinc-300 mb-2">Unique Key</label>
                <input
                  type="text"
                  name="key"
                  value={@editing_status.key}
                  class="w-full px-4 py-3 border border-zinc-300 dark:border-zinc-700 rounded-xl bg-zinc-50 dark:bg-zinc-800/50 focus:ring-2 focus:ring-fsm-primary outline-none transition-all font-mono text-sm"
            ...
            <div class="bg-zinc-50 dark:bg-zinc-950 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800">
              <label class="block text-sm font-semibold text-zinc-700 dark:text-zinc-300 mb-4">Status Color</label>
              <div class="flex items-center gap-8">
                ...
                />
                <div class="flex-1">
                   <p class="text-[10px] text-zinc-400 mb-2 uppercase font-bold tracking-widest">Live Visual</p>
                   <div class="px-5 py-3 rounded-xl border-2 shadow-sm flex items-center gap-3 bg-white dark:bg-zinc-800" style={"border-color: #{@editing_status.color}40"}>
                      <div class="size-3 rounded-full shadow-sm" style={"background-color: #{@editing_status.color}"}></div>
                      <span class="text-xs font-bold uppercase tracking-wider" style={"color: #{@editing_status.color}"}>
                         PREVIEW STATUS
                      </span>
                   </div>
                </div>
  ...
  def handle_event("save_all", _params, socket) do
    ...
    statuses = Enum.map(Enum.with_index(socket.assigns.statuses), fn {status, idx} ->
      %{
        "key" => get_field(status, :key, ""),
        "label" => get_field(status, :label, ""),
        "color" => get_field(status, :color, "#6B7280"),
