defmodule FieldHubWeb.TechLive.JobShow do
  ...
  def render(assigns) do
    ...
    <div class="mx-auto max-w-2xl bg-gray-50 min-h-screen pb-24">
      ...
      <div class="sticky top-0 z-10 bg-white border-b border-gray-200 px-4 py-3 flex items-center shadow-sm">
        <.link navigate={~p"/tech/dashboard"} class="mr-4 text-gray-500 hover:text-gray-700 transition-colors">
      ...
      <div class="p-4 space-y-4">
        ...
        <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
          <div class="flex justify-between items-start mb-2">
            <div>
              <p class="text-xs font-bold text-gray-400 uppercase tracking-wider">Job #<%= @job.number %></p>
        ...
        <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
          <h3 class="text-sm font-bold text-gray-900 mb-2 flex items-center">
            <.icon name="hero-document-text" class="w-4 h-4 mr-2 text-indigo-500" />
            Description
          </h3>
          <p class="text-gray-700 text-sm leading-relaxed"><%= @job.description || "No description provided." %></p>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div class="px-4 py-3 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
            <h3 class="text-sm font-bold text-gray-900 flex items-center">
               <.icon name="hero-user" class="w-4 h-4 mr-2 text-indigo-500" />
               Customer
            </h3>
            <div class="flex space-x-2">
              <a href={"tel:#{@job.customer.phone}"} class="p-2.5 bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-100 active:scale-95 transition-all">
                <.icon name="hero-phone" class="w-5 h-5" />
              </a>
              <a href={"#{map_link(@job)}"} target="_blank" class="p-2.5 bg-green-50 text-green-600 rounded-xl hover:bg-green-100 active:scale-95 transition-all">
          ...
          <div class="p-4">
            ...
            <div class="mt-4">
              <a href={"#{map_link(@job)}"} target="_blank" class="w-full flex justify-center items-center px-4 py-2.5 border border-indigo-600 shadow-sm text-sm font-bold rounded-xl text-indigo-600 bg-white hover:bg-indigo-50 active:scale-[0.98] transition-all">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200">
          <div class="px-4 py-3 border-b border-gray-100">
            <h3 class="text-sm font-bold text-gray-900 flex items-center">
              <.icon name="hero-clock" class="w-4 h-4 mr-2 text-indigo-500" />
              Service History
            </h3>
          </div>
          <div class="divide-y divide-gray-50">
            <%= if Enum.empty?(@history) do %>
              <div class="p-4 text-center text-sm text-gray-400">First time service at this location.</div>
            <% else %>
              <%= for h <- Enum.take(@history, 3) do %>
                <div class="p-4">
                  <div class="flex justify-between items-start mb-1">
                    <span class="text-xs font-bold text-gray-900"><%= h.title %></span>
                    <span class="text-[10px] text-gray-400 font-medium tracking-tight uppercase"><%= Calendar.strftime(h.inserted_at, "%m/%d/%Y") %></span>
                  </div>
                  <p class="text-xs text-gray-500 line-clamp-2"><%= h.work_performed || "No work details recorded." %></p>
                </div>
              <% end %>
            <% end %>
          </div>
  ...
  defp render_actions(assigns) do
    ...
    <%= case @job.status do %>
      <% "scheduled" -> %>
        <button phx-click="start_travel" class="flex-1 bg-indigo-600 text-white font-bold py-3.5 rounded-2xl shadow-lg shadow-indigo-200 active:scale-[0.98] transition-all">
          Start Travel
        </button>
      <% "dispatched" -> %>
        <button phx-click="start_travel" class="flex-1 bg-indigo-600 text-white font-bold py-3.5 rounded-2xl shadow-lg shadow-indigo-200 active:scale-[0.98] transition-all">
      ...
      <% "en_route" -> %>
        <button phx-click="arrive" class="flex-1 bg-purple-600 text-white font-bold py-3.5 rounded-2xl shadow-lg shadow-purple-200 active:scale-[0.98] transition-all">
          Arrived On Site
        </button>
      <% "on_site" -> %>
        <div class="flex gap-3 w-full">
          <button phx-click="start_work" class="flex-1 bg-yellow-500 text-white font-bold py-3.5 rounded-2xl shadow-lg shadow-yellow-200 active:scale-[0.98] transition-all text-sm">
            Start Work
          </button>
          <button phx-click="complete_job" class="flex-1 bg-green-600 text-white font-bold py-3.5 rounded-2xl shadow-lg shadow-green-200 active:scale-[0.98] transition-all text-sm">
            Complete
          </button>
        </div>
      <% "in_progress" -> %>
        <button phx-click="complete_job" class="flex-1 bg-green-600 text-white font-bold py-3.5 rounded-2xl shadow-lg shadow-green-200 active:scale-[0.98] transition-all">
          Complete Job
        </button>
      <% _ -> %>
  ...
  def handle_event("start_travel", _, socket) do
    case Jobs.start_travel(socket.assigns.job) do
      ...
      {:error, _} -> {:noreply, put_flash(socket, :error, "Could not update status")}
    end
  end
