defmodule FieldHubWeb.Components.PushNotificationHandler do
  ...
  def render(assigns) do
    show_banner = assigns[:permission] == "default" and not assigns[:dismissed] and assigns[:vapid_key]
    ...
    <div>
      <%= if @vapid_key do %>
        <div id="push-notification-handler"
      ...
      <%= if @show_banner do %>
        <div class="fixed bottom-4 right-4 z-50 bg-white dark:bg-zinc-800 shadow-lg rounded-lg p-4 border border-zinc-200 dark:border-zinc-700 max-w-sm transition-all duration-300 animate-in slide-in-from-bottom-2">
          <div class="flex items-start gap-3">
             ...
             <div class="flex-1">
               <h3 class="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Enable Notifications</h3>
               <p class="text-sm text-zinc-500 dark:text-zinc-400 mt-1">
                 Get real-time updates for new jobs and assignments.
               </p>
               <div class="mt-3 flex gap-2">
                 <button phx-click="request_permission" phx-target={@myself}
                         class="text-xs bg-blue-600 hover:bg-blue-700 text-white font-medium px-3 py-1.5 rounded-md transition-colors">
                   Enable
                 </button>
                 <button phx-click="dismiss_banner" phx-target={@myself}
             ...
             <button phx-click="dismiss_banner" phx-target={@myself} class="text-zinc-400 hover:text-zinc-500">
