defmodule FieldHubWeb.UserAuth do
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      FieldHubWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule FieldHubWeb.PageLive do
        ...
        on_mount {FieldHubWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{FieldHubWeb.UserAuth, :require_authenticated}] do
  ...
  def on_mount(:require_sudo_mode, _params, session, socket) do
    ...
    if Accounts.sudo_mode?(socket.assigns.current_scope.user, -30) do
      {:cont, socket}
    else
      socket =
        socket
